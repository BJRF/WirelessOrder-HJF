...
/**
 * Created by Lijunyuan on 2018/4/15.
 */

public class ClickListen extends Thread implements View.OnClickListener{
    ...
    public ClickListen(EditText name, EditText password,Activity act) {
        this.name = name;
...
//			s = new Socket("192.168.31.146", 8888);
            ...
            //告诉服务器自己是登录
            out.write("longin\r".getBytes());
            //传送IP端口给服务器

            out.flush();out.write((this.name.getText().toString().trim() + "|" + this.password.getText().toString().trim()+"\r").getBytes());
            ...
            if(result.toString().trim().equals("OK")){
                ...
                act.startActivity(intent);
                Looper.prepare();
                Toast.makeText(act,"用户"+name.getText().toString().trim()+"欢迎您的登录", Toast.LENGTH_LONG).show();
                Looper.loop();// 进入loop中的循环，查看消息队列
            }else {
                Looper.prepare();
                Toast.makeText(act,"账号或者密码错误", Toast.LENGTH_LONG).show();
                Looper.loop();// 进入loop中的循环，查看消息队列
            }
